...
class DTMDPPolicy(nn.Module):
    def __init__(
        ...
        self._dt = GPT(
            GPTConfig(action_dim=action_size, block_size=block_size, n_layer=n_layer, n_head=n_head, n_embd=n_emb, model_type='naive', max_timestep=max_timestep)
        )

    def forward(self, batch: dict):
        encoded_obs = self._observation_encoder(batch['observation_sequence_batch'])
        encoded_acts = self._actions_encoder(batch['action_sequence_batch'])
        timesteps = batch['observation_sequence_batch']['timestep'][:, -1:, :].long()
    ...
    def get_action(self, obs_seq: List[Observation], act_seq):
        ...
        dummy_batch = dict(observation_sequence_batch = obs_stacked,
                            action_sequence_batch = act_stacked)
        with torch.no_grad():
            actions = self.forward(dummy_batch).cpu().detach().numpy()
        return actions[0]
