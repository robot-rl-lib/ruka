...
class DetectAndTrackWrapper(gym.Wrapper):
    ...
    def reset(self):
        ...
        self._detect_and_track.reset(TargetObject(reference_image=ref_img))
    ...
    def _update_observation(self, obs):
        ...
        obs[Observe.TRACKER_OBJECT_BBOX.value] = np.array(box.to_axis_aligned().item().to_xyxy(), dtype=np.uint16)
        if self._save_bbox_img:
