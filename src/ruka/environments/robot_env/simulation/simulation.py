...
class World(gym.Env):
    ...
    def __init__(self, config, evaluate, test, validate, only_ball=False):
        ...
        if self.scene_type == SceneType.ON_TABLE:
            self._scene = scene.OnTable(self, config, self._rng, test, validate, only_ball=only_ball)
        elif self.scene_type == SceneType.ON_FLOOR:
        ...
        self._real_time = config.real_time
