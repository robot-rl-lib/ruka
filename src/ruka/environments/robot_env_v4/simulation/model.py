...
class Model(object):
    ...
    def load_model(self, path, start_pos=[0, 0, 0], 
        ...
        for i in range(self._physics_client.getNumJoints(self.model_id)):
            ...
            joint_limits = {'lower': joint_info[8], 'upper': joint_info[9],
                            'force': joint_info[10]}
            joints[i] = _Joint(self._physics_client, self.model_id, i, joint_limits, max_speed=self.max_speed)
    ...
    def delete(self):
        return self._physics_client.removeBody(self.model_id)
...
class _Joint(object):
    def __init__(self, physics_client, model_id, joint_id, limits, max_speed=1):
        self._physics_client = physics_client
        self.model_id = model_id
        self.jid = joint_id
        self.limits = limits
