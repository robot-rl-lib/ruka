...
def get_generic_path_information(paths, stat_prefix=''):
    ...
    for info_key in ['env_infos', 'agent_infos']:
        ...
        # if 'is_success' in all_env_infos:
        # total_successes = sum([max(x["is_success"]) for x in all_env_infos])
        # total_terminations = sum([max(x["terminals"]) for x in paths])
        # if total_terminations != 0:
        #     statistics[stat_prefix + 'success_rate'] = total_successes / total_terminations
        # else:
        #     statistics[stat_prefix + 'success_rate'] = total_successes / total_terminations
    return statistics
